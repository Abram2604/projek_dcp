...
Route::middleware('auth')->group(function () {
    ...
    Route::get('/notifikasi/baca/{id}', [NotifikasiController::class, 'markAsRead'])->name('notifikasi.read');
    Route::get('/notifikasi/baca-semua', [NotifikasiController::class, 'markAllRead'])->name('notifikasi.readAll');
    ...
    Route::get('/progja/rapbo/export', [App\Http\Controllers\RapboController::class, 'exportPdf'])->name('rapbo.export');
    Route::resource('rapbo', App\Http\Controllers\RapboController::class)->except(['show', 'create', 'edit']);

    Route::middleware('role:BPH')->group(function() {
        Route::get('/keuangan', [KeuanganController::class, 'index'])->name('keuangan.index');
        Route::post('/keuangan/saldo-awal', [KeuanganController::class, 'storeSaldoAwal'])->name('keuangan.saldo_awal');
        Route::post('/keuangan/pemasukan', [KeuanganController::class, 'storePemasukan'])->name('keuangan.pemasukan');
    });

    Route::get('/laporan-keuangan', [LaporanKeuanganController::class, 'index'])->name('laporan_keuangan.index');
    Route::post('/laporan-keuangan', [LaporanKeuanganController::class, 'store'])->name('laporan_keuangan.store');

    Route::get('/laporan-keuangan/export/pdf', [LaporanKeuanganController::class, 'exportPdf'])
        ->name('laporan_keuangan.pdf');

    Route::post('/laporan-keuangan/ttd', [LaporanKeuanganController::class, 'updateTtd'])->name('laporan_keuangan.update_ttd');

    Route::middleware('role:BPH')->group(function () {
        Route::get('/pengaturan-ttd', [TtdController::class, 'edit'])->name('ttd.edit');
        Route::post('/pengaturan-ttd', [TtdController::class, 'update'])->name('ttd.update');
    });

    Route::middleware('akses.puk')->group(function () {
        ...
        Route::get('/data-anggota/export', [DataAnggotaController::class, 'exportExcel'])->name('data_anggota.export');
    });
